...
This guide describes how to create a template file that uses [`nmm`](https://github.com/a2-ai/nonmem-monitor)
...
This template file only uses the `nmm` binary location and the NONMEM model config toml file. The toml file sets the configuration of the `nmm` and is explained in the [`generate_nmm_config` reference](/reference/generate_nmm_config_reference)
